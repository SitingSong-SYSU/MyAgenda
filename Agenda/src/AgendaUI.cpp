...
void AgendaUI::printMeetings(std::list<Meeting> t_meetings) {
  std::cout << std::setiosflags(std::ios::left) << std::setfill(' ')
            << std::setw(3) << "no." << " | " << std::setw(8) << "title"
            << " | " << std::setw(8) << "sponsor" << " | "
            << std::setw(17) << "start time" << " | " << std::setw(17)
            << "end time" << " | " << std::setw(15) << "participator" << std::endl;
  std::cout << std::setiosflags(std::ios::left) << std::setfill('-')
            << std::setw(80) << "-" << std::endl;
  int cnt = 0;
  std::list<std::string> t_title;
  std::list<std::string> t_sponsor;
  std::list<std::string> t_participator;
  for (auto iter = t_meetings.begin(); iter != t_meetings.end(); iter++) {
    cnt++;
    for (int i = 0; i < iter->getTitle().size(); i += 8) {
      if (i + 8 <= iter->getTitle().size())
        t_title.push_back(iter->getTitle().substr(i, 8));
      else
        t_title.push_back(iter->getTitle().substr(i, iter->getTitle().size() - i));
    }
    for (int i = 0; i < iter->getSponsor().size(); i+= 8) {
      if (i + 8 <= iter->getSponsor().size())
        t_sponsor.push_back(iter->getSponsor().substr(i, 8));
      else
        t_sponsor.push_back(iter->getSponsor().substr(i, iter->getSponsor().size() - i));
    }
    bool flag = false;
    std::string tmp;
    ...
    for (int i = 0; i < tmp.size(); i+= 13) {
      if (i + 13 <= tmp.size())
        t_participator.push_back(tmp.substr(i, 13));
      else
        t_participator.push_back(tmp.substr(i, tmp.size() - i));
    }
    int ti = t_title.size(), sp = t_sponsor.size(), pa = t_participator.size();
    int maxSize = (ti > sp) ? ti: sp;
    maxSize = ((maxSize > pa) ? maxSize : pa);
    std::string t_ti, t_sp, t_pa;
    for (int i = 0; i < maxSize; i++) {
      if (i < ti) {
        t_ti = t_title.front();
        t_title.pop_front();
      ...
      if (i < sp) {
        t_sp = t_sponsor.front();
        t_sponsor.pop_front();
      } else {
        t_sp = " ";
      }
      if (i < pa) {
        t_pa = t_participator.front();
        t_participator.pop_front();
      } else {
        t_pa = " ";
      }
      if (i == 0)
        std::cout << std::setiosflags(std::ios::left) << std::setfill(' ')
                  << std::setw(3) << cnt << " | " << std::setw(8) << t_ti  << " | " 
                  << std::setw(8) << t_sp << " | "
                  << std::setw(17) << Date::dateToString(iter->getStartDate()) << " | "
                  << std::setw(17) << Date::dateToString(iter->getEndDate()) << " | " 
                  << std::setw(15) << t_pa << std::endl;
      else
        std::cout << std::setiosflags(std::ios::left) << std::setfill(' ')
                  << std::setw(3) << " " << " | " << std::setw(8) << t_ti  << " | " 
                  << std::setw(8) << t_sp << " | " << std::setw(17) << " " << " | "
                  << std::setw(17) << " " << " | " << std::setw(15) << t_pa << std::endl;
    }
    // std::cout << std::setiosflags(std::ios::left) << std::setfill(' ')
